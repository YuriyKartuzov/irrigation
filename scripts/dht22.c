 ...
 * Compile with: gcc -Wall dht22.c -o dht22.exe -lwiringPi
 * Run with: ./DHT22
 */
...
void    read_DHT_Data()  {
    ...
    delay(1.5);  // was 18
    digitalWrite( DHT_PIN, HIGH );
    delayMicroseconds(1);      //was 40,  should be no more than 15 as sensor pulls down the sygnal line
...
int main( void ) {
    ...
    for (int i=0; i<5000; i++) {
        read_DHT_Data();
        delay( 2000 );  /* Wait 1.5 seconds between readings. */
    }
